angular.module('Directorio', ['ngMaterial', 'ngRoute', 'ui.bootstrap']);
...
angular.module('Directorio').controller('aplicacion', ['$scope', 'DirectorioService', function ($scope, DirectorioService) {
    ...
    $scope.CargarLista = function () {
        DirectorioService.ConsultaEmpleados(function (response) {
            ...
            $scope.groupToPages($scope.ListaCompletaColaboradores);
        ...
        var Token = 'lol';
        DirectorioService.CatalogoUbicaciones(Token, function (response) {
            var todos = { id: 0, codigo: '', descripcion: 'TODOS' };
            $scope.ubicaciones = response.CatalogoUbicacionesResult.Lista;
            $scope.ubicaciones.push(todos);

        });
        DirectorioService.CatalogoAreas(Token, function (response) {
            var todos = { id: 0, codigo: '', descripcion: 'TODOS' };
            $scope.areas = response.CatalogoAreasResult.Lista;
            $scope.areas.push(todos);
        });
    };
    $scope.CargarLista();
    $scope.busqueda = { nombre: '', departamento: { id: 0, codigo: '', descripcion: 'TODOS' }, ubicacion: { id: 0, codigo: '', descripcion: 'TODOS' } };
    $scope.BuscarColaborador = function (busqueda) {
        var lista = [];
        for (i = 0; i < $scope.ListaCompletaColaboradores.length; i++) {
            if ($scope.ListaCompletaColaboradores[i].Nombre.toLowerCase().includes(busqueda.nombre.toLowerCase())) {
                ...
                else if(busqueda.departamento.id != 0){
                    if($scope.ListaCompletaColaboradores[i].Departamento.id === busqueda.departamento.id)
                        lista.push($scope.ListaCompletaColaboradores[i]);
                }
                else {
